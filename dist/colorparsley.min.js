...
parsley:function(c){var f=0,g=0,h=10,n=100;for(k=1;k<c.length;k++)c[k]&&(f=f?f:k,g=k);switch(g){case 9:h=16;case 1:b[0]=b[1]=b[2]=parseInt(c[g],h);break;case 12:h=16,n=255;case 4:b[3]=c[g].match(/%/g)||12==g?parseInt(c[g],h)/n:parseFloat(c[g]);case 11:h=11==g?16:h;case 3:for(i=0;3>i;i++)b[i]=parseInt(c[f+i],h);break;case 8:n=15,b[3]=parseInt(c[g],16)/n;case 7:h=16;for(i=0;3>i;i++)b[i]=parseInt(c[f+i]+c[f+i],h);break;case 17:b[3]=c[g].match(/%/g)?parseInt(c[g],h)/n:parseFloat(c[g]);case 16:b[5]=c[f];
for(i=0;3>i;i++)f++,b[i]=c[f]?c[f].match(/%/g)?parseInt(c[f])/100:parseFloat(c[f]):1;if(b[5].match(/^(?:hsla?|hwba?)/i)){c=function(p){p=(p+r/30)%12;return q-t*Math.min(q,1-q)*Math.max(-1,Math.min(p-3,9-p,1))};var r=b[0]%360;0>r&&(r+=360);if(b[5].match(/^hsla?/i)){var t=b[1];var q=b[2];var l=0;var m=1}else if(b[5].match(/^hwba?/i)){l=b[1];m=b[2];if(1<=l+m){b[0]=b[1]=b[2]=l/(l+m);b[5]="sRGB";break}t=1;q=.5;m=1-l-m}b[0]=Math.round(255*(c(0)*m+l));b[1]=Math.round(255*(c(8)*m+l));b[2]=Math.round(255*

...
/////           Beta 0.1.8   Revision date: February 09, 2026
/////
...
function parseString(colorString = '#abcdef') {
  ...
  if (colorString.match(/^(?:(?!rgb|l.h|hs|col|okl|\d|#).{0,4})(?=[g-z])/)) {
    ...
    parsley: function (slices) {
      ...
      switch (sliceLast) {
        ...
        case 22: // This is the "wild west" section
          ...
          // Process for OKLCH -> OKLAB
          else if (retArray[5].match(/^ok(?:lch|lab)a?/i)) {
            let L, a, b;

            // OKLCH -> OKLAB conversion if needed
            if (retArray[5].match(/^oklcha?/i)) {
              L = retArray[0]; // 0-1
              let C = Math.abs(retArray[1]); // chroma must be positive
              let H = retArray[2]; // hue degrees
              ...
              L = retArray[0]; // 0-1
              a = retArray[1]; // -0.4 to 0.4 typically
              b = retArray[2]; // -0.4 to 0.4 typically
            }
            ...
            retArray[0] = Math.round(255 * Math.max(0, Math.min(1, toSRGB(r_lin))));
            retArray[1] = Math.round(255 * Math.max(0, Math.min(1, toSRGB(g_lin))));
            retArray[2] = Math.round(255 * Math.max(0, Math.min(1, toSRGB(b_lin))));
            retArray[5] = 'sRGB';
          }

...
/////           Beta 0.1.2   Revision date: Jan 15, 2022
/////
...
// @output_file_name colorparsley.0.1.2.min.js
// @code_url https://raw.githubusercontent.com/Myndex/colorparsley/master/src/colorparsley.0.1.2.js
// ==/ClosureCompiler==
...
////////////////////////////////////////////////////////////////////////////////
/////  BEGIN COLOR PARSLEY 0.1.2  BLOCK  \/////////////////////////////////////
////                                      \///////////////////////////////////
///                                        \/////////////////////////////////
...
function colorParsley (colorIn) {
    ...
    if (typeof colorIn === 'string') {
       ...
       if (Array.isArray(colorIn)) {
          return colorIn;
       } else if (colorIn.r || colorIn.red){ // validate object & return array
         let objArray = [0,0,0,'',false,'unknown'];
                       // takes object with r g b or red green blue etc...
         objArray[0] = (colorIn.r)?colorIn.r:(colorIn.red)?colorIn.red:0;
         objArray[1] = (colorIn.g)?colorIn.g:(colorIn.green)?colorIn.green:0;
         objArray[2] = (colorIn.b)?colorIn.b:(colorIn.blue)?colorIn.blue:0;
        ...
        return [0,0,0,'',false,'inputError'] // throw 'InvalidInput' // return error 
};
...
function parseString (colorString = '#abcdef') {
    ...
    this.isValid = false; // validation flag, in 5th array element.
    this.type = 'sRGB'; // Default colorspace flag in 6th element.
    
        // test for named color before iterating array (optimization)
    if (colorString.match(/^(?:(?!rgb|l.h|hs.|#).+)(?=[G-Z])/i)) {

      colorString = colorString.toLowerCase();   // set lowercase

      ///// CSS4 NAMED COLORS plus GREYS /////////////
  ...
  let retArray = [0,0,0,'',this.isValid,this.type];
       ...
       // NEW regex parse  0.1.2
      // See docs for breakdown of regex pattern
  let colorRex = {
    rex: /(?:^(?:rgba?|)\(?\s?(?:(25[0-5]|2[0-4]\d|1?\d{1,2})(?:, ?| ))(?:(25[0-5]|2[0-4]\d|1?\d{1,2})(?:, ?| )(?:(25[0-5]|2[0-4]\d|1?\d{1,2}))(?:(?:,| \/| )\s?(?:(100%|\d?\d%|[01]?\.\d{1,22}|[01])))?(?: \)| |))?(?!\S)|^(?:#|0x|)(?:(?:([\da-f])([\da-f])([\da-f])([\da-f])?)(?!\S)|(?:([\da-f]{2})(?:([\da-f]{2})([\da-f]{2})([\da-f]{2})?)?))(?!\S)|^(?:([^r\d\W][a-z]{2,5})\(?\s?((?:\d{0,3}\.|)\d+%?)(?:, ?| )((?:\d{0,3}\.|)\d+%?)(?:, ?| )((?:\d{0,3}\.|)\d+%?)(?:(?:,| ?\/) ?((?:\d{0,3}\.|)\d+%?))?)(?: \)| |))$/i,
    ...
    parsley: function (slices) {
      ...
      switch (sliceLast) {
        case 9:    // allows two digit hex to become grey
          base = 16;
        case 1:  // allows 1-3 digit INT with comma to become grey
        ...
        case 17:  // this section is for hsv type parsings 
          retArray[3] = (slices[sliceLast].match(/%/g)) ?
           parseInt(slices[sliceLast],base) / divisor : parseFloat(slices[sliceLast]);
        case 16:
       ...
       // return retArray;  // no need to return as we set array directly
...
//  The main call
    ...
    if (slicesProc) { // Error catch
      ...
      return [0,0,0,'',this.isValid,'parsleyError']  // throw 'InvalidString'
    }
};
...
/////  ƒ  colorToHex()  ///////////////////////////////////////////////////
    ...
    // hex2 — returns hex string, 3,4,6, or 8 chars if that was entered, no #
    // If alpha is 1 or empty, no alpha is returned i.e. abcf returns abc
    
//export
function colorToHex (rgba = [0,0,0,'']) {
